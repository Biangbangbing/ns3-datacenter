source config.sh
configFile=$NS3/examples/WRdma/config-burst.txt
RES_DUMP=$NS3/examples/WRdma/dump_burst

mkdir $RES_DUMP

algs=(0 1 2 3 4 5 6)

algNames=("dcqcn" "powerInt" "hpcc" "powerDelay" "timely" "dctcp" "wrdma")
CCMODE=(1 3 3 3 7 8 15)
...
for algorithm in ${algs[@]};do
	...
	#echo "time ./waf --run "wrdma-evaluation-fairness --conf=$configFile --algorithm=${CCMODE[$algorithm]} --wien=$wien --delayWien=$delay --windowCheck=$window""
	time ./waf --run "wrdma-evaluation-burst --conf=$configFile --algorithm=${CCMODE[$algorithm]} --wien=$wien --delayWien=$delay --windowCheck=$window" > $RESULT_FILE  2> $RESULT_FILE &
done
